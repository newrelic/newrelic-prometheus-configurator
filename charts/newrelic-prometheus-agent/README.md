...
| config.kubernetes.curated_experience.app_values | list | `["redis","mongodb","consul","calico","ingress-nginx"]` | app_values to be used in the relabel configs when looking for the values of labels of source_labels. |
| config.kubernetes.curated_experience.enabled | bool | `true` | enabling the curated experience the jobs, selected by jobs_prefix, scrapes merely the targets having one of the specified labels matching    # one of the values of app_values. |
...
| config.kubernetes.curated_experience.source_labels | list | `["app.kubernetes.io/name","app.newrelic.io/name"]` | source_labels to be used in the relabel configs when looking for app_values. |
